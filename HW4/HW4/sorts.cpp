...
//  HW4
//
//  Created by Quentin Truong on 3/4/17.
...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int unsorted = 1; unsorted < s.size(); ++unsorted){
        ...
        int loc = unsorted;
        for (; loc > 0 && comp(next, s[loc - 1]); --loc)
            s[loc] = s[loc - 1];
        s[loc] = next;
    }
}
...
int main()
{
    ...
    // destruction time.
    {
        ...
        for (int a = 0; a < auxStudents.size(); a++)
            ptrStudents.push_back(&(auxStudents[a]));
